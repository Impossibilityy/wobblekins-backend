// =============================================================================
// wobblelab-backend/api/stripe-webhook.js   (ESM)
// -----------------------------------------------------------------------------
// Stripe calls this after payment. We verify the signature, mark the Supabase
// order paid (idempotently), ensure order items exist, store the fulfillment
// method explicitly, and email the customer + admin via Resend.
//
// FULFILLMENT (added):
//   method  <- session.metadata.fulfillment_method (explicit; never inferred
//              from whether an address exists), falling back to the value saved
//              on the order, then 'shipping'.
//   local_pickup -> shipping_cents = 0, pickup_status = 'preparing',
//                   shipping_address = null, pickup_location from env; sends the
//                   "pickup confirmed" email instead of the shipping email.
//   shipping     -> unchanged (address + shipping rate preserved).
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ---- fulfillment method: EXPLICIT, never inferred from address ----------
    const fulfillmentMethod =
      (session.metadata && session.metadata.fulfillment_method) ||
      order.fulfillment_method || 'shipping';
    const isPickup = fulfillmentMethod === 'local_pickup';
    console.log('  fulfillment method:', fulfillmentMethod);

    // ---- build the paid-update payload --------------------------------------
    const updatePayload = {
      status: 'paid',
    ...
    if (isPickup) {
      updatePayload.shipping_cents = 0;
      updatePayload.shipping_address = null;            // pickup never stores an address
      updatePayload.pickup_status = 'preparing';
      updatePayload.pickup_location = process.env.PICKUP_LOCATION || null;
    } else {
      updatePayload.shipping_cents = (session.total_details && session.total_details.amount_shipping) || 0;
    ...
    if (customerEmail) {
      try {
        await resend.emails.send({
          ...
          to: customerEmail,
          subject: isPickup
            ? 'Your Wobblekin pickup order is confirmed'
            : 'Your Wobblekin adoption is confirmed',
          html: isPickup ? pickupConfirmedEmailHtml(paidOrder, items) : customerEmailHtml(paidOrder, items)
        });
        console.log('  customer email: sent to', customerEmail, isPickup ? '(pickup)' : '(shipping)');
      } catch (e) {
    ...
    try {
      await resend.emails.send({
        from: FROM_EMAIL,
        to: ADMIN_EMAIL,
        subject: `New ${isPickup ? 'PICKUP' : 'shipping'} order · ${money(paidOrder.total_cents)} · ${customerEmail || 'no email'}`,
...
// --- NEW: local-pickup order confirmation -----------------------------------
function pickupConfirmedEmailHtml(order, items) {
  const name = order.customer_name ? escapeHtml(String(order.customer_name).split(' ')[0]) : 'friend';
  return `
  <div style="margin:0;padding:0;background:#000000;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#000000;padding:24px 12px;">
      <tr><td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#060608;border:1px solid rgba(255,255,255,.12);border-radius:18px;overflow:hidden;font-family:Arial,Helvetica,sans-serif;">
          ...
          <tr><td style="padding:30px 28px 8px;">
            <div style="font-size:12px;letter-spacing:3px;text-transform:uppercase;">${rainbowWordmark()}</div>
            <h1 style="margin:14px 0 6px;color:#f4f4f8;font-size:24px;line-height:1.2;">Your pickup order is confirmed</h1>
            <p style="margin:0;color:#b6b6c2;font-size:15px;line-height:1.6;">
              Hi ${name}, your payment was successful and your order is being prepared at the Lab. Pickup is free.
            </p>
          </td></tr>
          <tr><td style="padding:18px 28px 6px;">
            <div style="background:#0d0d12;border:1px solid rgba(255,255,255,.10);border-radius:14px;padding:16px 18px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr><td style="padding:14px 0 0;color:#9a9aa6;font-size:13px;">Subtotal</td>
                    <td align="right" style="padding:14px 0 0;color:#f4f4f8;font-size:13px;">${money(order.subtotal_cents)}</td></tr>
                <tr><td style="padding:4px 0;color:#9a9aa6;font-size:13px;">Pickup</td>
                    <td align="right" style="padding:4px 0;color:#42d60c;font-size:13px;">Free</td></tr>
                ${order.tax_cents ? `<tr><td style="padding:4px 0;color:#9a9aa6;font-size:13px;">Tax</td><td align="right" style="padding:4px 0;color:#f4f4f8;font-size:13px;">${money(order.tax_cents)}</td></tr>` : ''}
                <tr><td style="padding:10px 0 0;color:#f4f4f8;font-size:16px;font-weight:bold;">Total paid</td>
                    <td align="right" style="padding:10px 0 0;color:#ffd23f;font-size:16px;font-weight:bold;">${money(order.total_cents)}</td></tr>
              </table>
      ...
      <tr><td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#060608;border:1px solid rgba(255,255,255,.12);border-radius:14px;overflow:hidden;font-family:Arial,Helvetica,sans-serif;">
          ...
          <tr><td style="padding:24px 26px;">
            <h2 style="margin:0 0 14px;color:#f4f4f8;font-size:19px;">New ${isPickup ? 'LOCAL PICKUP' : 'shipping'} order</h2>
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="color:#cfcfd8;font-size:14px;line-height:1.7;">
              <tr><td style="color:#9a9aa6;width:150px;">Fulfillment</td><td style="color:#f4f4f8;font-weight:bold;">${isPickup ? 'Local Pickup' : 'Shipping'}</td></tr>
              <tr><td style="color:#9a9aa6;">Customer</td><td style="color:#f4f4f8;">${escapeHtml(order.customer_name || '—')}</td></tr>
              ...
              ${isPickup
                ? `<tr><td style="color:#9a9aa6;">Pickup status</td><td style="color:#f4f4f8;">${escapeHtml(order.pickup_status || 'preparing')}</td></tr>`
